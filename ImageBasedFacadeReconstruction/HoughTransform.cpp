...
/**
 * Compute the Hought accumulator.
 *
 * @param image						image
 * @param accum						accumulator
 * @param use_magnitude_as_weight	use the gradient magnitude as weight if true
 * @param grad						gradient magnitude
 */
void HoughTransform::computeAccumulator(cv::Mat image, cv::Mat& accum, bool use_magnitude_as_weight, cv::Mat grad) {
	// compute the accumulation
	double hough_h = ((sqrt(2.0) * (double)(image.rows > image.cols ? image.rows : image.cols)) / 2.0);

	accum = cv::Mat(hough_h * 2, 180, CV_32F, cv::Scalar(0.0f));
	double center_x = image.cols / 2.0;
	double center_y = image.rows / 2.0;
	for (int y = 0; y < image.rows; ++y) {
		for (int x = 0; x < image.cols; ++x) {
			if (image.at<uchar>(y, x) < 255) continue;

			for (int t = 0; t < 180; ++t) {
				double r = ((double)x - center_x) * cos((double)t / 180.0 * M_PI) + ((double)y - center_y) * sin((double)t / 180.0 * M_PI);
				if (use_magnitude_as_weight) {
					accum.at<float>(r + hough_h, t) += grad.at<float>(y, x);
				}
				else {
	...
	cv::Point minIdx;
	cv::Point maxIdx;
	cv::minMaxLoc(accum, &minVal, &maxVal, &minIdx, &maxIdx);
	accum *= 255.0 / maxVal;
}

void HoughTransform::suppressNonMaximum(cv::Mat accum, cv::Mat& result, int size, float threshold) {
	accum.copyTo(result);

	for (int r = 0; r < accum.rows; ++r) {
		for (int c = 0; c < accum.cols; ++c) {
			float value = 0.0f;
			if (accum.type() == CV_32F) {
				value = result.at<float>(r, c);
			}
			else if (accum.type() == CV_64F) {
				value = result.at<double>(r, c);
			}

			if (!isLocalMaximum(accum, r, c, size) || value < threshold) {
				if (accum.type() == CV_32F) {
					result.at<float>(r, c) = 0;
				}
				else if (accum.type() == CV_64F) {
					result.at<double>(r, c) = 0;
				}
			}
		}
	}
...
void HoughTransform::saveAccum(const std::string& filename, const cv::Mat& accum) {
	double minVal;
	double maxVal;
	cv::Point minIdx;
	cv::Point maxIdx;
	...
	cv::imwrite(filename.c_str(), (accum - minVal) / (maxVal - minVal) * 255);
}

void HoughTransform::computeLines(const cv::Mat& accum, std::vector<cv::Vec4b>& lines) {

}

bool HoughTransform::isLocalMaximum(const cv::Mat& mat, int r, int c, int size = 4) {
...
void HoughTransform::visualizeAccum(const cv::Mat& image, const cv::Mat& accum, int max_degree, const std::string& filename) {
	...
	for (int r = 0; r < accum.rows; ++r) {
		for (int t = 0; t < accum.cols; ++t) {
			...
			if (accum.at<float>(r, t) > 0) {
				...
				if (t >= 45 && t <= 135) {
					x1 = 0;
					y1 = ((double)(r - (accum.rows / 2)) - ((x1 - (image.cols / 2)) * cos(t / 180.0 * M_PI))) / sin(t / 180.0 * M_PI) + (image.rows / 2);
					x2 = image.cols;
					y2 = ((double)(r - (accum.rows / 2)) - ((x2 - (image.cols / 2)) * cos(t / 180.0 * M_PI))) / sin(t / 180.0 * M_PI) + (image.rows / 2);
				}
				else {
					y1 = 0;
					x1 = ((double)(r - (accum.rows / 2)) - ((y1 - (image.rows / 2)) * sin(t / 180.0 * M_PI))) / cos(t / 180.0 * M_PI) + (image.cols / 2);
					y2 = image.rows;
					x2 = ((double)(r - (accum.rows / 2)) - ((y2 - (image.rows / 2)) * sin(t / 180.0 * M_PI))) / cos(t / 180.0 * M_PI) + (image.cols / 2);
				}
				cv::line(result, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(255, 0, 0), 1);
			}
		}
	}
