...
namespace fs {
	...
	void getSplitLines(const cv::Mat_<float>& mat, int size, float threshold, std::vector<float>& split_positions);
	void getSplitLines2(const cv::Mat_<float>& mat, float threshold, std::vector<float>& split_positions);
